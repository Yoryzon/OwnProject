...
class RenderProcess(multiprocessing.Process):
    ...
    def run(self):
        ...
        bucketArray = numpy.ndarray(shape=(self.bucketSize, self.bucketSize, 3), dtype=float)
        bucketArray.fill(0)  # Very import need to set default color

        # shoot multiple rays each pixel for antialiasing
            ...
            for j in range(bucketY, bucketY + self.bucketSize):
                ...
                for i in range(bucketX, bucketX + self.bucketSize):
                    ...
                    camRayEnd = self.cam.pos + Vector((i + AAsampleGrid[thisAAoffset][0] - self.width / 2) * focusRatio,
                                                      (-j - AAsampleGrid[thisAAoffset][
                                                          1] + self.height / 2) * focusRatio,
        ...
        print(
            "Process Finished - " + multiprocessing.current_process().name + " Render time: " + str(processRenderTime))
        sys.stdout.flush()
    ...
    def getRefractionColor(self, currObj, prevHitPos, hitResult, indirectDepth, refractDepth, reflectDepth,
        ...
        if refractDepth == 1:
            ...
            reflectionCol = reflectionCol + self.getMirrorReflectionColor(currObj, prevHitPos, hitResult, indirectDepth,
                                                                          reflectDepth, reflectDist)

        if refractionHitBool:
            reflectDist = reflectDist + (
                        refractHitResult[1] - hitResult[1]).length()  # accumulation of distance of reflection
            prevHitPos = hitResult[1]
            hitResult = refractHitResult
            if refractDepth < self.refractionMaxDepth:
                refractionCol = refractionCol + self.getColor(hitResult, prevHitPos, indirectDepth=indirectDepth,
                                                              refractDepth=refractDepth, reflectDist=reflectDist)
            else:
    ...
    def getMirrorReflectionColor(self, currObj, prevHitPos, hitResult, indirectDepth, reflectDepth, reflectDist):
        ...
        if reflectionHitBool:
            reflectDist = reflectDist + (
                        reflectHitResult[1] - hitResult[1]).length()  # accumulation of distance of reflection
            prevHitPos = hitResult[1]
            hitResult = reflectHitResult
            if reflectDepth < self.reflectionMaxDepth:
                reflectionCol = reflectionCol + self.getColor(hitResult, prevHitPos, indirectDepth=indirectDepth,
                                                              reflectDepth=reflectDepth,
                                                              reflectDist=reflectDist).colorMult(
                    currObj.material.reflectionColor)
            else:
                reflectionCol = reflectionCol + self.getHitPointColor(hitResult).colorMult(
                    currObj.material.reflectionColor)
    ...
    def getColor(self, hitResult, prevHitPos, indirectDepth=0, reflectDepth=0, refractDepth=0, reflectDist=0):
        ...
        if currObj.material.refractionWeight == 1 and currObj.material.reflectionWeight == 1:
            ...
            hitPointColor = hitPointColor + self.getRefractionColor(currObj, prevHitPos, hitResult, indirectDepth,
                                                                    refractDepth, reflectDepth, reflectDist)
        elif currObj.material.refractionWeight != 1 and currObj.material.reflectionWeight == 1:
            # If this object is perfect mirror
            hitPointColor = hitPointColor + self.getMirrorReflectionColor(currObj, prevHitPos, hitResult, indirectDepth,
                                                                          reflectDepth, reflectDist)
        else:
            # Diffuse material
            if self.kernel == 0:
            ...
            if indirectDepth < self.indirectDepthLimit:
                ...
                for i in range(self.indirectSamples):
                    ...
                    indirectRayDir = hitResult[2].rot("A", tangentRotAmount, tangentAxis).rot("A", biTrangentRotAmount,
                                                                                              hitResult[2])
                    ...
                    if indirectHitBool:
                        ...
                        indirectHitPColor = self.getColor(indirectHitResult, hitResult[1], indirectDepth, 0, 0,
                                                          reflectDist)  # get the indirect color
                ...
                hitPointColor = hitPointColor + indirectColor.colorMult(
                    matColor) * 0.3  # arbitrary contribution multiplier
        ...
        for eachLight in self.scene.lights:
            for i in range(eachLight.samples):
                if "AreaLight" in eachLight.type:
                    ...
                    if not eachLight.isDoubleSided:
                ...
                if lambert > 0:
                    ...
                    offsetOrigin = hitResult[
                                       1] + shadowRayDir.normalized() * self.bias  # slightly offset the ray start point because the origin itself is a root
                    ...
                    if not inShadow:
                        litColor = litColor + eachLight.color * (
                                    eachLight.intensity * lambert / (4 * math.pi * math.pow(temp_t, 2)))
