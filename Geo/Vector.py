...
class Vector:
    ...
    def rot(self, axis, angle, axisVec=None):
        ...
        if axisVec is None:
            ...
            Vz = axisVec.z
            # arbitrary
            rotAMatrix = np.array([[cosA + math.pow(Vx, 2) * (1 - cosA), Vx * Vy * (1 - cosA) + Vz * sinA,
                                    Vx * Vz * (1 - cosA) - Vy * sinA],
                                   [Vy * Vx * (1 - cosA) - Vz * sinA, cosA + math.pow(Vy, 2) * (1 - cosA),
                                    Vy * Vz * (1 - cosA) + Vx * sinA],
                                   [Vz * Vx * (1 - cosA) + Vy * sinA, Vz * Vy * (1 - cosA) - Vx * sinA,
                                    cosA + math.pow(Vz, 2) * (1 - cosA)]])
