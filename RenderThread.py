...
class RenderThread(QThread):
    ...
    def __init__(self, width, height, scene, cam):
        ...
        self.scene = scene
        self.cam = cam
        self.canvas = np.ndarray(shape=(self.height, self.width, 3), dtype=float)
        ...
        if self.bucketOrder == 1:
            ...
            while len(shuffledBucketList) < len(bucketSplitData):
                for i in range(stepAmount):
                    ...
                    if len(bucketSplitData) > currIndex >= 0:
    ...
    def run(self):
        ...
        for i in range(processCnt):
            job = RenderProcess(jobsQueue, self.width, self.height, bucketPosData, bucketCnt, bucketCntLock,
                                self.bucketSize, self.scene, self.cam)
            jobs.append(job)
        ...
        while True:
            ...
            if bucketDataSet == "Done":
                ...
                bucketBufferImage = QImage(bucketBufferArray.data, self.bucketSize, self.bucketSize,
                                           bucketBufferArray.strides[0], QImage.Format_RGB888)
                ...
                with bucketCntLock:
                    if bucketCnt.value < len(bucketPosData) * bucketDataSet[4] - 1:
                        ...
                        self.bucketProgressSignal.emit(
                            [nextLocatorPosX, nextLocatorPosY, self.bucketSize, dataX, dataY])
